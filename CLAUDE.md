...
- **純粋なドメインロジックは`features/<object>/domain/`に置く**。UIにもfetchにも依存しない関数(例: 合計計算、ステータス遷移の判定)であり、composable/コンポーネントから明示importして使う。
  - 根拠: SRP(取得・状態管理と業務規則の分離)とテスト容易性のため。プロパティベーステスト(fast-check)の主対象。
